...
Open [http://localhost:3001](http://localhost:3001). Port 3001 is used by default so another local application on port 3000 cannot be mistaken for Touchline AI. Override it with `LOCAL_WEB_PORT` when needed.
...
New edits use **AI-selected natural length between 60 and 120 seconds for complete recaps** (or the full usable source when the upload itself is shorter than 60 seconds). There is no fixed 65/75/85-second target. The 60-120 contract is applied before discovery, so Gemini inventories enough reserve footage before local verification. Automatic edits do not add ending padding or synthetic replay merely to reach the estimate.

The processor follows an evidence-first sequence before writing content:

1. **Observe:** Gemini reviews the complete source timeline from beginning to end in timing-keyed excerpts and records complete actions at their natural boundaries. It prioritizes confirmed goals, shots on target, saves, shots, and big chances. Each goal is requested as one shared story ID containing the live finish, immediate celebration, and replay proof. Temporary quota, timeout, and service responses preserve progress and retry automatically.
...
The processor counts only unique, renderable source time toward the 60-second floor. It initially locks a duration buffer and promotes already-discovered reserves after local verification instead of returning to whole-video discovery. Incomplete Gemini structured responses are retried in compact batches and omitted IDs are never treated as scene rejections. It does not relax ball/player visibility or camera stability checks.

Retries are exclusive per job: another analysis or render retry receives HTTP 409 while that video is queued or processing. Pre-lock retries resume from the saved passage inventory and revalidate stale semantic decisions; only jobs that already passed Step 1 restore the immutable locked-scene manifest. Gemini 429, 408, 502, 503, 504, deadline, socket, and fetch failures save progress and retry persistently with visible backoff state. Alignment keeps the verified scene order, including silent goal celebrations.
...
Gemini semantic verification version 9 uses compact labeled evidence reels, an explicit clip manifest, and exact response-ID completeness checks. It requires explicit visible payoff times and a source-frame result region. Goal/shot captions, outcome sound accents, goal color changes and payoff teasers require that region and continuously observed evidence. Unverified outcome effects are omitted; they are never replaced with guessed scoring claims.
