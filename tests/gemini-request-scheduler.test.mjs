...
test("processor persists quota waits and retries 429 responses without a fixed attempt ceiling", () => {
  ...
  assert.match(source, /seconds \+ duration > 45 \|\| batch\.length >= 4/);
  assert.match(source, /Gemini omitted \$\{missingIds\.length\} scene review/);
  assert.match(source, /saved candidates were not falsely rejected/);
  assert.match(source, /const staleSemanticReview = candidates\.filter/);
  assert.match(source, /\.\.\.staleSemanticReview/);
  assert.match(source, /if \(quotaLimited\) \{[\s\S]*quotaAttempt \+= 1;[\s\S]*continue;/);
  assert.match(source, /temporary timeout\/service response/);
  assert.doesNotMatch(source, /transientAttempt >= attempts/);
...
test("Gemini discovery is confined to Step 1 before scene-by-scene tracking", () => {
  const source = readFileSync(new URL("../local-processor/server.mjs", import.meta.url), "utf8");
  assert.match(source, /const savedStep1Scenes = trackingOnlyRetry \? await loadStep1SceneManifest/);
  const processStart = source.indexOf("async function processJob");
  const trackingStart = source.indexOf("let prepared = await prepareIncidentEvidence", processStart);
  const timelineLockStart = source.indexOf("buildLockedVisualOutline(scriptCandidates", trackingStart);
  const stepTwo = source.slice(trackingStart, timelineLockStart);
  assert.ok(processStart >= 0 && trackingStart > processStart && timelineLockStart > trackingStart);
  ...
  assert.match(stepTwo, /promoteRemainingScenesForLock/);
});
