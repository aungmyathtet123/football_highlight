...
export function buildFfmpegArgs(input: FfmpegRenderInput): string[] {
  ...
  input.segments.forEach((segment, index) => {
    ...
    filters.push(`[0:v]trim=start=${segment.sourceStart.toFixed(3)}:duration=${duration.toFixed(3)},setpts=PTS-STARTPTS,crop=ih*9/16:ih:${xExpression}:0,scale=1080:1920:flags=lanczos,setsar=1[v${index}]`);
